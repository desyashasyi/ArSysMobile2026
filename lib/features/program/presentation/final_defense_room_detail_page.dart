...
class FinalDefenseRoomDetailPage extends ConsumerWidget {
  final int eventId;
  final int roomId;
  final String roomName;

  const FinalDefenseRoomDetailPage({super.key, required this.eventId, required this.roomId, required this.roomName});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final detailAsync = ref.watch(programFinalDefenseRoomDetailProvider((eventId: eventId, roomId: roomId)));
        ...
        error: (err, _) => Center(
          child: Column(
            ...
            children: [
              ...
              ElevatedButton.icon(
                onPressed: () => ref.invalidate(programFinalDefenseRoomDetailProvider((eventId: eventId, roomId: roomId))),
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text('No applicants in this room', style: TextStyle(color: Colors.grey[500])),
                ],
              ),
            );
          }
          return RefreshIndicator(
            onRefresh: () => ref.refresh(programFinalDefenseRoomDetailProvider((eventId: eventId, roomId: roomId)).future),
...
class _ApplicantScoreCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final examiners = applicant['examiner_scores'] as List<dynamic>? ?? [];
    final supervisors = applicant['supervisor_scores'] as List<dynamic>? ?? [];
    ...
    return Card(
      ...
      child: Container(
        ...
        child: ExpansionTile(
          ...
          children: [
            if (examiners.isNotEmpty) _scoreSection(context, 'Examiner Scores', examiners, Colors.deepPurple, showRole: false),
    ...
    return Column(
      ...
      children: [
        ...
        ...scores.map((s) {
          ...
          final name = sc['staff_name'] ?? sc['examiner_name'] ?? '-';
