...
import 'package:arsys/features/program/presentation/pre_defense_score_detail_page.dart';

class PreDefenseScoresPage extends ConsumerWidget {
  const PreDefenseScoresPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final eventsAsync = ref.watch(programPreDefenseEventsProvider);

    return Scaffold(
      appBar: AppBar(title: const Text('Pre-Defense Scores')),
        ...
        error: (err, _) => Center(
          child: Column(
            ...
            children: [
              ...
              Text('$err', style: TextStyle(color: Colors.grey[600], fontSize: 13), textAlign: TextAlign.center),
              const SizedBox(height: 16),
              ElevatedButton.icon(
                onPressed: () => ref.invalidate(programPreDefenseEventsProvider),
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text('No pre-defense events', style: TextStyle(color: Colors.grey[500])),
                ],
              ),
            );
          }
          return RefreshIndicator(
            onRefresh: () => ref.refresh(programPreDefenseEventsProvider.future),
                    ...
                    onTap: () {
                      ...
                      if (id != null) {
                        Navigator.of(context).push(MaterialPageRoute(
                          builder: (_) => PreDefenseScoreDetailPage(eventId: id, eventCode: ev['event_id_string'] ?? ''),
                        ));
                      }
                    },
                    child: Container(
