...
class Page extends Component
{
    ...
    use WithPagination;
    use LivewireAlert;
    protected $listeners = ['refreshPage_ArSysSpecializationEventApplicant_FinalDefense' => '$refresh', 'refreshPage'];
    public function render()
    {
        ...
        if(!is_null(Auth::user()->staff->program->cluster)){
            ...
            ->where('event_type_id', EventType::where('examination_type', 'Final-defense')->where('code', 'PUB')->first()->id)
            ->whereHas('finaldefenseApplicant')
            ->orderBy('event_date', 'DESC')
            ->paginate($perPage = 10, $columns = ['*'], $pageName = 'eventDatePage');
        }else{
            $events = Event::where('program_id', $programId)
            ->where('event_type_id', EventType::where('examination_type', 'Seminar')->where('code', 'SEM')->first()->id)
        ...
        return view('livewire.specialization.event.final-defense.page', ['events' => $events]);
    }
    ...
    public function publishEvent($eventId){
        if(!is_null(Event::find($eventId)->unsetFinalDefenseRoom)){
            if(Event::find($eventId)->finaldefenseApplicant->contains('room_id', null)){
                ...
                foreach(Event::find($eventId)->finaldefenseApplicant as $applicant){
                    if(!is_null($applicant->room)){
                        if(is_null(EventApplicantFinalDefense::find($applicant->id)->publish)){
                            EventApplicantFinalDefense::find($applicant->id)->update([
                                'publish' => 1,
                            ]);
                            foreach($applicant->research->supervisor as $supervisor){

                                if(is_null(FinalDefenseSupervisorPresence::
                                    ...
                                    FinalDefenseSupervisorPresence::create([
