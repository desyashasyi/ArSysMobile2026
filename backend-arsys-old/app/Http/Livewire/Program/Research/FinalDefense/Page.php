...
use App\Exports\FinalDefenseMark;
...
class Page extends Component
{
    ...
    public function render()
    {
        ...
        $events = Event::where('program_id', $programId)
            ->where('event_type_id', EventType::where('examination_type', 'Final-defense')->first()->id)
        ...
        return view('livewire.program.research.final-defense.page', ['events' => $events]);
    }
    ...
    public function exportMark(){
        return Excel::download(new FinalDefenseMark($this->eventId), Auth::user()->staff->program->code.'-'.Auth::user()->staff->program->abbrev.'-FinalDefenseMark.xlsx');
    }
